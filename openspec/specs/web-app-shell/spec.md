...
Every page SHALL render a header containing the application wordmark linking to the home page, a link to the products page, and the API status indicator; and a footer. The home page SHALL present the application name and a primary action leading to the products page.

#### Scenario: Header on every page
- **WHEN** a user opens the home page, the products page, a product's detail page or a product form
- **THEN** the same header with the wordmark, the `Products` link and the API status is visible at the top
